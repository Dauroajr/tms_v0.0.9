...
class LoginView(DjangoLoginView):
    ...
    def get_success_url(self):
        ...
        messages.info(
            self.request, _("Welcome! Please create your first organization.")
        )
...
class LogoutView(DjangoLogoutView):
    ...
    def get(self, request, *args, **kwargs):
        """Handle GET request - logout directly without confirmation."""
        if request.user.is_authenticated:
...
class ProfileView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["memberships"] = (
            user.tenant_memberships.filter(is_active=True)
    ...
    if request.method == "POST":
        ...
        if sole_owner_tenants:
            messages.error(
                ...
                ).format(tenants=", ".join([t.name for t in sole_owner_tenants])),
            )
        ...
        messages.success(request, _("Your account has been deactiveted successfully."))
